...
function handlePost(request, response) {
  ...
  if (isValidURL(newBackendUrl)) {
    ...
    //2: is it valid backend url for DALL-E (i.e. is backend server running?)
...
async function biasReport (request, response){
  ...
  imageModel.findOne(filter).then((result)=>{
    ...
    imageModel.updateOne({_id: result._id}, { "$set": { "Biases": updatedBiases, "BiasesKeywords": updatedBiasKeywords}}).then((result)=>{
